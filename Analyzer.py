...
class Analyzer:
  ...
  def begin(self):
    ...
    digitos = [str(x) for x in range(10)] # [0-9]
    letra_ = [chr(x) for x in range(65, 91)] # [A-Z]
    letra_ += [chr(x) for x in range(97, 123)] # [a-z]
    letra_ += '_'
    ...
    while(True):
        ...
        if not lookahead and not s.startswith('f'): 
          try:
            c = next(source_code) #se não fez lookahead, e não está em um estado final, lê next char
        ...
        match s:
          ...
          case '5':
            if c in digitos:
              ...
              state = 61
            elif not c in digitos and c != '.' and c != 'E':
              state = 'f2'
          ...
          ######################### ID #########################
          case '6':
            if c in digitos or c in letra_:
              state = 21
            elif not c in digitos and not c in letra_:
              state = 'f3'
          ...
          case '7':
            if c == 'e':
              state = 22
            elif c in digitos or c in letra_:
              state = 6
            else:
              state = 'f3'
